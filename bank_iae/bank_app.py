...
def create_bank_payment():
    ...
    if va_checked:
        response = requests.put('https://paymentiae.azurewebsites.net/updatebankpayment', json={
            'addAmount': amount,
...
def update_bank_status():
    ...
    if 'addAmount' in json_data:
        ...
        response = requests.put('https://paymentiae.azurewebsites.net/updatepaymentstatus', json={
            'va': va,
            'status': upd_val
        })
